...
// static int disk_wrapper_holder;
...
static int __init disk_wrapper_init(void) {
  ...
  // struct file *bdev_file, *flags_bdev_file;
  ...
  #elif (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 12, 0))
    // struct blk_holder_ops holder;
    printk(KERN_INFO "target device path : %s\n", strim(target_device_path));
  ...
  #error "Unsupported kernel version: CrashMonkey has not been tested with " \
  ...
  #endif
  #if (LINUX_VERSION_CODE == KERNEL_VERSION(6, 8, 0))
    struct bdev_handle *flags_bdev_handle;
    flags_bdev_handle = bdev_open_by_path(strim(flags_device_path), BLK_OPEN_READ, NULL, NULL);
      ...
      flags_bdev_file = bdev_file_open_by_path(strim(flags_device_path), BLK_OPEN_READ, NULL, NULL);
  ...
  if (!flags_device->bd_queue) {
    ...
    unsigned int flush_flags = flags_device->bd_queue->flush_flags;
  #endif
  ...
  printk(KERN_INFO "After assigning\n");
  printk(KERN_INFO "Queue flags: %lu\n", Device.gd->queue->queue_flags);
  printk(KERN_INFO "Queue Data: %p\n", Device.gd->queue->queuedata);

  printk(KERN_INFO "hwm: working with queue with:\n\tflags 0x%lx\n", Device.gd->queue->queue_flags);
  // printk(KERN_INFO "hwm: target device \n\tflags 0x%lx\n", queue_flags);
...
static void __exit hello_cleanup(void) {
  printk(KERN_INFO "Starting clean up ....\n");
  free_logs();
  printk(KERN_INFO "Free_logs done...\n");
  #if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 12, 0) && \
      LINUX_VERSION_CODE < KERNEL_VERSION(3, 14, 0)) || \
    (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0) && \
      LINUX_VERSION_CODE < KERNEL_VERSION(3, 17, 0)) || \
  ...
  printk(KERN_INFO "fput bdev file done ...\n");
  ...
  if (bdev_file) {
    ...
    printk(KERN_INFO "put_disk done...\n");
  #else
    blk_cleanup_queue(Device.gd->queue);
  #endif
  del_gendisk(Device.gd);
  printk(KERN_INFO "del_gendisk done....\n");
  
  put_disk(Device.gd);
  printk(KERN_INFO "put disk done .... \n");
